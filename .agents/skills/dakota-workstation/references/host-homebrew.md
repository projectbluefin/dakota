...
ChairLift (Brewfile, desktop assets, polkit policy, config and the
`frostyard/tap` migration) is owned by `projectbluefin/common` and shipped
unmodified. Fix ChairLift behavior in common, not with a Dakota overlay.
