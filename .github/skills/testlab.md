...
`bootc switch <tag>` silently does nothing if the tag resolves to the already-booted digest. Always force the upgrade with the exact digest:

```bash
DIGEST=$(curl -sI http://<build-host-ip>:5000/v2/dakota/manifests/<TAG> \
  -H 'Accept: application/vnd.oci.image.manifest.v1+json' \
  | grep -i docker-content-digest | awk '{print $2}' | tr -d '\r')
sudo bootc switch --transport registry <build-host-ip>:5000/dakota@${DIGEST}
```
...
Skip the nvidia variant for local builds to cut build time:
