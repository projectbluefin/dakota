...
- [ ] `strip-binaries: ""` set if binary requires setuid (`4755`) permissions
...
### Setuid binaries require `strip-binaries: ""` to preserve `4755` permissions (2026-07-26)

When packaging setuid binaries (like `sudo-rs`), BuildStream's default post-install
`strip-binaries` pass (`freedesktop-sdk-stripper`) strips debug symbols from installed
binaries in-place, which resets file permissions and clears the setuid (`4755`) bit down
to `0755`. This causes `sudo` to fail at runtime because it lacks the setuid root bit
needed to elevate privileges.

Always add `variables: strip-binaries: ""` to elements that install setuid binaries:
