...
| Primary read cache | `gbm.gnome.org:11003` — GNOME's server; carries most BST graph artifacts (read-only for dakota) |
| Secondary read cache | `cache.projectbluefin.io:11001` — dakota-specific artifact reads |
| CI write / remote-exec | `cache.projectbluefin.io:11002` — mTLS; artifact push, storage-service, source-cache push, and (x86 only) remote execution |
...
**Two-tier read path (from `project.conf`):**
- `gbm.gnome.org:11003` is queried **first** for artifact and source reads. Most of the BST dependency graph (all of GNOME, freedesktop-sdk) lives here — dakota does not build it, it fetches it. This server is read-only for dakota.
- `cache.projectbluefin.io:11001` is the secondary read source for dakota-specific artifacts.

**CI write path (`buildstream-ci.conf` generated by `generate-bst-ci-config` action):**
- `cache.projectbluefin.io:11002` (mTLS) — used for artifact push, source-cache push, storage-service, and (x86 only) remote execution + action cache.
- aarch64 jobs pass `enable-remote-execution: false` — they push artifacts to `:11002` but do **not** use the remote execution or action-cache services. Local builds only with `max-jobs` defaulting to the scheduler `builders: 4` value.
- x86 jobs pass `enable-remote-execution: true` — `max-jobs: 32` routes compilation to the remote-execution cluster.

**Implication for concurrent builds:**
- Two concurrent **x86** builds contend for `:11002` remote-execution bandwidth and runner slots — the serialisation rule in Hard Rule 9 applies.
- A concurrent **x86 + aarch64** build still shares the `:11002` artifact-push path but does NOT share the remote-execution cluster. Contention is lower, but still real. The rule still applies.
